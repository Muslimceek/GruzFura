...
import { User, Zap, History, Bell, Moon, Shield, Star, Award, Settings, ChevronRight, LogOut, Lock, LogIn, FileEdit, Trash2 } from 'lucide-react';
...
export const ProfileView: React.FC<ProfileViewProps> = ({ currentUser, listings, onDelete, onEdit, onStatusChange, onLogout, onLoginRequest }) => {
    const { t } = useLanguage();
    ...
    const SettingToggle = ({ label, icon: Icon, color, active, onToggle }: { label: string, icon: any, color: string, active: boolean, onToggle: () => void }) => (
        <button onClick={onToggle} className="w-full flex items-center justify-between p-4 group active:scale-[0.99] transition-transform text-left">
    ...
    if (!currentUser || currentUser.isAnonymous) {
      return (
        <div className="h-full flex flex-col items-center justify-center px-8 bg-[#F3F4F6] text-center">
            <div className="w-24 h-24 rounded-[2rem] bg-white shadow-xl flex items-center justify-center mb-6 text-slate-300">
                <Lock size={40} />
            </div>
            <h2 className="text-2xl font-black text-slate-900 mb-2">{t('profile_protected_title')}</h2>
            <p className="text-slate-500 font-medium mb-8">{t('profile_protected_desc')}</p>
            <button 
              onClick={onLoginRequest}
              className="px-8 py-4 rounded-2xl bg-slate-900 text-white font-black flex items-center gap-3 shadow-xl active:scale-95 transition-all"
            >
              <LogIn size={20} />
              {t('profile_login_btn')}
            </button>
        ...
        <div className="h-full overflow-y-auto no-scrollbar bg-[#F3F4F6] relative">
            <div className="relative pt-safe-top px-6 pb-20 overflow-hidden bg-[#0B1120]">
                ...
                <div className="relative z-10 flex flex-col items-center mt-6 animate-in slide-in-from-top-4 fade-in duration-700">
                    <div className="relative mb-6 group cursor-pointer">
                        ...
                        <div className="w-28 h-28 rounded-full bg-gradient-to-b from-slate-800 to-slate-950 border-[3px] border-white/10 flex items-center justify-center relative shadow-2xl overflow-hidden group-hover:scale-105 transition-transform duration-300">
                             {currentUser.photoURL ? <img src={currentUser.photoURL} className="w-full h-full object-cover" /> : <User size={48} className="text-white/80" strokeWidth={1.5} />}
                        ...
                        <div className="absolute bottom-1 right-1 w-8 h-8 bg-emerald-500 rounded-full border-[4px] border-[#0B1120] flex items-center justify-center text-white shadow-lg">
                            <Zap size={14} fill="currentColor" />
                        </div>
                    </div>

                    <h2 className="text-3xl font-black text-white tracking-tight mb-1 drop-shadow-md">{currentUser.displayName || t('guest_user')}</h2>
                    <p className="text-white/40 text-[10px] font-bold uppercase tracking-[0.2em] mb-3">{currentUser.email}</p>
                ...
                <div className="relative z-10 grid grid-cols-3 gap-3 mt-8 animate-in slide-in-from-bottom-4 fade-in duration-700 delay-100">
                    <StatItem label="Рейтинг" value="5.0" icon={Star} color="from-yellow-400/20 text-yellow-400" />
                    <StatItem label="Посты" value={myListings.length.toString()} icon={Award} color="from-blue-500/20 text-blue-500" />
                    <StatItem label="Доверие" value="100%" icon={Shield} color="from-emerald-400/20 text-emerald-400" />
                </div>
            </div>
            ...
            <div className="relative -mt-10 rounded-t-[2.5rem] bg-[#F3F4F6] min-h-[60vh] px-6 py-8 space-y-8 z-20 shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.1)]">
                ...
                <section>
                    <div className="flex items-center justify-between mb-5 px-2">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-xl font-black text-slate-900 tracking-tight">{t('profile_my_posts')}</h3>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {myListings.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-10 bg-white rounded-[2rem] border border-dashed border-slate-200">
                                <p className="text-slate-400 font-bold text-sm">{t('profile_no_posts')}</p>
                            </div>
                        ) : (
                            myListings.map((l) => (
                                <div key={l.id} className="relative group">
                                    ...
                                    <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                        <button 
                                          onClick={() => onEdit(l)}
                                          className="w-10 h-10 rounded-xl bg-white shadow-lg border border-slate-100 text-blue-600 flex items-center justify-center active:scale-90 transition-all"
                                        >
                                          <FileEdit size={18} />
                                        </button>
                                        <button 
                                          onClick={() => onDelete(l.id)}
                                          className="w-10 h-10 rounded-xl bg-white shadow-lg border border-slate-100 text-red-500 flex items-center justify-center active:scale-90 transition-all"
                                        >
                                          <Trash2 size={18} />
                                        </button>
                                    </div>
                    ...
                    <div className="space-y-4">
                        {recentListings.length === 0 ? (
                            ...
                            recentListings.map((l) => (
                                <ListingCard key={`recent-${l.id}`} listing={l} currentUserId={currentUser.uid} onDelete={onDelete} />
                            ))
                        )}
                    </div>
                </section>
                ...
                <div className="pt-4 pb-24 space-y-3">
                    <button 
                      onClick={onLogout}
                      className="w-full py-4 rounded-2xl text-red-500 font-bold text-xs uppercase tracking-widest hover:bg-red-50 transition-colors flex items-center justify-center gap-2"
                    >
                        <LogOut size={16} />
                        {t('profile_logout')}
                    </button>
